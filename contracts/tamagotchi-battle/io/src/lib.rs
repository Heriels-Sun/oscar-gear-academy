...
use store_io::{StoreEvent, StoreAction, AttributeId};
...
impl Battle {
    pub async fn register(&mut self, tmg_id: &TamagotchiId) {
        ...
        let actual_side = if random_position == 0 {
            SidewayMovement::Left
        } else {
            SidewayMovement::Right
        };
    ...
    pub fn make_move(&mut self, direction: SidewayMovement, attribute: Option<AttributeId>) {
        assert_eq!(
            self.state,
            BattleState::Moves,
        ...
        match attribute {
            Some(attribute_to_use) => {
                player.actual_attribute = attribute_to_use;
            }
            None => {
                player.actual_attribute = 0;
            }
        }
        ...
        if !self.shields_data.contains_key(&player.actual_attribute) {
    ...
    pub fn tamagotchi_total_atack(&self, player: &Player, opponent: &Player) -> u16 {
        ...
        let player_weapon_attribute = player.actual_attribute;
        let opponent_attribute = opponent.actual_attribute;
        
        if self.weapons_data.contains_key(&player_weapon_attribute) {
            total_attack *= *self.weapons_data.get(&player_weapon_attribute).expect("Unable to get the weapon attribute");
        }
        
        if self.shields_data.contains_key(&opponent_attribute) {
            let protection = *self.shields_data.get(&opponent_attribute).expect("Unable to get the shield attribute");
    ...
    pub fn make_reservation(&mut self, reservation_amount: u64, reservation_duration: u32) {
        ...
        msg::reply(BattleEvent::GasReserved, 0)
            .expect("Unable to send the message `BattleEvent::GasReserved`");
    }
}
...
pub struct Player {
   ...
   pub actual_attribute: u32,
   pub actual_side: SidewayMovement
}
...
pub enum BattleAction {
    ...
    Move(SidewayMovement, Option<AttributeId>),
    UpdateInfo,
    StartNewGame,
    ReserveGas {
...
// MAX AND MIN
const MAX_POWER: u16 = 10000;
const MIN_POWER: u16 = 3000;
const MAX_ENERGY: u16 = 60000;
const MIN_ENERGY: u16 = 20000;
const GAS_TO_USE: u64 = 1000000000; // * 0.1 TVara
const TIME_FOR_UPDATE: u32 = 20;       // * 1 minute 
const MAX_STEPS_FOR_ROUND: u8 = 4;

// Weapons
pub const SWORD_NORMAL_POWER: Power    = 2;
pub const SWORD_NORMAL_ID: AttributeId = 1;

pub const SWORD_RARE_POWER: Power    = 5;
pub const SWORD_RARE_ID: AttributeId = 2;

pub const SWORD_MYTHICAL_POWER: Power    = 7;
pub const SWORD_MYTHICAL_ID: AttributeId = 3;

pub const SWORD_LEGENDARY_POWER: Power    = 9;
pub const SWORD_LEGENDARY_ID: AttributeId = 4;

pub const BOW_NORMAL_POWER: Power    = 2;
pub const BOW_NORMAL_ID: AttributeId = 5;

pub const BOW_RARE_POWER: Power    = 6;
pub const BOW_RARE_ID: AttributeId = 6;

pub const BOW_MYTHICAL_POWER: Power    = 8;
pub const BOW_MYTHICAL_ID: AttributeId = 7;

pub const BOW_LEGENDARY_POWER: Power    = 10;
pub const BOW_LEGENDARY_ID: AttributeId = 8;

// Shields

pub const SHIELD_NORMAL_PROTECTION: Protection = 2000;
pub const SHIELD_NORMAL_ID: AttributeId = 10;

pub const SHIELD_RARE_PROTECTION: Protection = 4000;
pub const SHIELD_RARE_ID: AttributeId = 11;

pub const SHIELD_MYTHICAL_PROTECTION: Protection = 5500;
pub const SHIELD_MYTHICAL_ID: AttributeId = 12;

pub const SHIELD_LEGENDARY_PROTECTION: Protection = 6000;
pub const SHIELD_LEGENDARY_ID: AttributeId = 13;