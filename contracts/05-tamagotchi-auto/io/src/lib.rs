...
use gstd::{prelude::*, ActorId, exec, msg, ReservationId};
...
impl Tamagotchi {
    ...
    pub async fn buy_attribute(&mut self, store: &ActorId, attribute: u32) {
    ...
    pub fn make_reservation(&mut self, amount_to_reserve: u64, duration_of_reserve: u32) {
    ...
    pub fn tmg_needs_something(&self) -> Option<TmgEvent> {
        ...
        let rested = self.slept.saturating_sub(self.calculate_energy());
        if fed == HUNGER_TO_SEND_MSG {
            Some(TmgEvent::FeedMe)
        }else if entertained == BOREDOM_TO_SEND_MSG {
            Some(TmgEvent::PlayWithMe)
        }else if rested == ENERGY_TO_SEND_MSG {
            Some(TmgEvent::WantToSleep)
        }else {
            None
        }
    }
...
pub const HUNGER_TO_SEND_MSG: u64 = 10;
pub const BOREDOM_TO_SEND_MSG: u64 = 20;
pub const ENERGY_TO_SEND_MSG: u64 = 20;
